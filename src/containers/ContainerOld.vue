      ...
      <div v-for="(cellComponent, indexCol) in colComponentList" :key="indexCol" v-if="cellComponent.ifCondition ? evaluateString(cellComponent.ifCondition, cellComponent, data, null, null) : true" :style="[cellComponent.style]" :class="[cellComponent.isGroup?'group-cell':null, cellComponent.noMargin? 'm-0': null, cellComponent.noPadding? 'p-0': null, cellComponent.align?'align-' + cellComponent.align: null, cellComponent.width?'col-' + cellComponent.width:'col', cellComponent.offset?'offset-' + cellComponent.offset:null]">
        <template v-for="(component, indexCell) in cellComponent.content" v-if="component.ifCondition ? evaluateString(component.ifCondition, component, data, null, null) : true">
          ...
          <span v-else-if="component.type === 'containerArray'" :key="indexCell">
            <template v-for="indexPart in getObjectFromString(component.model)">
          ...
          <span v-else-if="component.type === 'label' && !component.text && component.model" :class="{'float-right':component.h_align==='right'}" :key="indexCell">
            <label :id="component.id" :style="[component.style, {margin: 0}]">{{getObjectFromString(component.model+dataIndex+(component.key?'.'+component.key:''), '')}}</label>
          </span>
          <span v-else-if="component.type === 'datelabel' && !component.text && component.model" :class="{'float-right':component.h_align==='right'}" :key="indexCell">
            <label :id="component.id" :style="[component.style, {margin: 0}]">{{datetimeToString(getObjectFromString(component.model+dataIndex+(component.key?'.'+component.key:''), ''))}}</label>
          </span>
          ...
          <span v-else-if="component.type === 'imagelabel' && !component.source && component.imageModel" :class="{'float-right':component.h_align==='right'}" :key="indexCell">
            <b-img-lazy :id="component.id" :style="component.style" :src="getObjectFromString(component.imageModel+dataIndex, 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN8Vw8AAmEBb87E6jIAAAAASUVORK5CYII=')" :rounded="component.shape==='circle'?'circle':'0'" :width="component.imgWidth" :height="component.imgHeight" />
            <label :id="component.id" :style="[{margin: 0}]">{{getObjectFromString(component.model+dataIndex, '')}}</label>
          </span>
          ...
          <span v-else-if="component.type === 'filtertextfield'" :class="{'float-right':component.h_align==='right'}" :key="indexCell">
            <b-form-input :id="component.id" type="text" @input="updateFilterTextfield(component, $event)" :style="component.style" :placeholder="component.placeholder"/>
          </span>
          ...
          <span v-else-if="component.type === 'dropdown' && component.data" :class="{'float-right':component.h_align==='right'}" :key="indexCell">
            <b-form-select :id="component.id" :name="component.model+dataIndex" :value="dropdownValue(data[component.model+dataIndex])" @input="dropdownInput(component.model+dataIndex, component, $event)" :options="component.data" :style="component.style" :placeholder="component.placeholder"></b-form-select>
          </span>
          ...
          <span v-else-if="component.type === 'imagedropdown' && component.data" class="image-dropdown" :class="{'float-right':component.h_align==='right'}" :key="indexCell">
            ...
            <b-img :src="data[component.model+dataIndex]?data[component.model+dataIndex].imgUrl:'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN8Vw8AAmEBb87E6jIAAAAASUVORK5CYII='" fluid style="float: left;width: 130px; height: 80px"/>
          ...
          <span v-else-if="component.type === 'blockbutton'" :key="indexCell" style="width: 100%">
            <b-button :id="component.id" @click="btnClick(component.action, component)" style="width: 100%" :style="[component.style, component.style == null || component.style.width == null ? {minWidth: '120px'}:null]"><i v-if="component.icon" :class="[component.icon]"/> {{component.text | translate}}</b-button>
          </span>
          ...
          <span v-else-if="component.type === 'cardhorizontal2'" :class="{'card-horizontal2':true, 'float-right':component.h_align==='right'}" :key="indexCell">
            <b-card :style="component.style" class="p-0" style=" margin-bottom: 0.5em; width: 100%">
              <b-img-lazy class="card-image-left" style="width: 10em; height:10em; margin-bottom: 0; border-radius: 10px 0 0 10px; " src="https://placekitten.com/g/400/450" />
              <span class="card-text" style="display: inline-block; padding: 1em;">
                <span style="font-weight: bold;">{{component.title}}</span><br/>
                {{component.text1}} <br/>
                {{component.text2}} <br/>
                {{component.text3}} <br/>
          ...
          <span v-else-if="component.type === 'mapPicker'" :key="indexCell">
            <b-input-group>
              <b-form-input :id="component.id" type="text" readonly @click.native="mapPickerClick(component.model+dataIndex, component)" :name="component.model+dataIndex" :value="arrayToString(data[component.model+dataIndex])" v-validate="!component.validation ? '' : component.validation.join('|')" :data-vv-as="component.label | translate" :state="errors.has(component.model+dataIndex) ? false : null" :style="[component.style, {borderRadius: '5px 0 0 5px', backgroundColor: 'white'}]" :placeholder="'map_selectlocation' | translate"/>
          ...
          <span v-else-if="component.type === 'mapBoundary'" :key="indexCell">
            <gmap-map
              :ref="component.id"
              :center="{lat:-6.2152408, lng:106.8301251}"
              :zoom="12"
              ...
              :key="indexCell">
              <gmap-polygon v-if="data[component.model] && data[component.model].length > 0" :path="data[component.model]" ref="polygon">
              </gmap-polygon>
            </gmap-map>
          </span>
          <span v-else-if="component.type === 'mapLineArray'" :key="indexCell">
            <gmap-map
              ...
              :key="indexCell">
              <gmap-polygon 
                v-for="indexPart in data[component.modelCount]" 
                :key="indexPart" 
                v-if="data[component.modelFrom + '_' + indexPart] && data[component.modelFrom + '_' + indexPart].boundary.length > 0" 
                :path="data[component.modelFrom + '_' + indexPart].boundary" 
                ref="polygon1">
              </gmap-polygon>
              <gmap-polygon 
                v-for="indexPart in data[component.modelCount]" 
                :key="indexPart" 
                v-if="data[component.modelTo + '_' + indexPart] && data[component.modelTo + '_' + indexPart].boundary.length > 0" 
                :path="data[component.modelTo + '_' + indexPart].boundary" 
                ref="polygon2">
              </gmap-polygon>
              <gmap-polyline v-for="indexPart in data[component.modelCount]" 
               v-if="data[component.modelFrom + '_' + indexPart] && data[component.modelFrom + '_' + indexPart].boundary.length > 0 && data[component.modelTo + '_' + indexPart] && data[component.modelTo + '_' + indexPart].boundary.length > 0"
              :path="[getMiddlePoint(data[component.modelFrom + '_' + indexPart].boundary), getMiddlePoint(data[component.modelTo + '_' + indexPart].boundary)]" :key="indexPart">
              </gmap-polyline>
            </gmap-map>
          </span>
          ...
          <span v-else-if="component.type === 'table' && fields[component.id] != null" :key="indexCell">
            <b-table :ref="component.id"
              ...
              <template v-for="field in fields[component.id]" :slot="'HEAD_' + field.key" v-if="field.key != 'actions'" slot-scope="data">
                ...
                <input v-if="field.filter" type="text" @click.stop=";" v-model="shared.filter[component.id][field.key]" @keyup="filterTyped(component)" :key="field.key" style="width: 100%;"/>
              </template>
...
export default {
  ...
  mounted () {
    ...
    for (let row = 0; row < this.rowComponentList.length; row++) {
      ...
      for (let col = 0; col < colComponentList.length; col++) {
        ...
        for (let cellNum = 0; cellNum < cellComponentList.length; cellNum++) {
          ...
          if ((component.type === 'dropdown' || component.type === 'radiobutton' || component.type === 'checkbox') && component.source) {
            ...
            if (component.criteria) {
              ...
              if (processedCrit) {
                ...
                Vue.set(this.shared.filter, component.id, filterCrit)
              } else {
                Vue.set(this.shared.filter, component.id, {})
              }
            } else {
              Vue.set(this.shared.filter, component.id, {})
            }
          ...
          if (component.ref || component.type === 'table' || component.type === 'map' || component.type === 'mapBoundary' || component.type === 'mapLineArray' || component.type === 'mapArray') {
    ...
    this.$nextTick(() => {
      ...
      for (let i = 0; i < this.shared.components.length; i++) {
        ...
        let refMap = this.$refs[comp.id]
        if (refMap) {
          Object.assign(this.shared.refs, this.$refs)
        }
        if (comp.type === 'map' || comp.type === 'mapBoundary' || comp.type === 'mapLineArray' || comp.type === 'mapArray') {
  ...
  methods: {
    myProvider (ctx, callback) {
      ...
      let apiUrl = token[1]
      let empty = []
      let filter = this.shared.filter[compId]
    ...
    filterTyped (component) {
      this.shared.refs[component.id][0].refresh()
    },
    ...
    processAction (action, component, item, index, urlParam) {
      ...
      switch (action.type) {
        case 'refreshTable':
          this.shared.refs[action.componentId][0].refresh()
          break
        case 'getData':
      ...
      if (component.source) {
        ...
        if (option) {
          ...
          if (component.refreshId) {
            ...
            if (comp.type === 'map' || comp.type === 'mapBoundary' || comp.type === 'mapLineArray') {
              this.refreshMap(comp)
            }
          }
        }
    ...
    updateFilterTextfield (component, value) {
      // if (this.shared.filter && this.shared.filter[component.componentId]) {
      this.shared.filter[component.componentId][component.key] = value
      // }
    },
    ...
    mapPickerClick (model, component) {
      ...
      this.selectMapLocation({
        ...
        selectLocationCallback: (path) => {
          Vue.set(instance.data, model, path)
    ...
    datetimeToString (datetime, formatParam) {
      let format = 'YYYY-MM-DD h:mm:ss'
