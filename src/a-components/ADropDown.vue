    ...
    fetchOption () {
      if (this.attr.source) {
        ...
        if (this.attr.source.method && this.attr.source.method === 'post') {
          api.post(this.$util.stringInject(this.attr.source.url, {data: this.$store.state.generic.data[this.dataId], props: this.$props, _user: this.$store.state.user}, this.dataId), {},
            (response) => {
          ...
          api.get(this.$util.stringInject(this.attr.source.url, {data: this.$store.state.generic.data[this.dataId], props: this.$props, _user: this.$store.state.user}, this.dataId),
            (response) => {
              this.optionList = response.data
            },
            () => {
